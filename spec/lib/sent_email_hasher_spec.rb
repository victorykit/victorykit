...
describe SentEmailHasher do
  ...
  describe "#member_for" do
    it "should return sent_email instance if the hash is valid and sent_email is present" do
      ...
      SentEmailHasher.member_for(SentEmailHasher.generate(sent_email.id)).should == sent_email
    end
    it "should return nil if the hash is valid but sent_email is not present" do
      SentEmailHasher.member_for(SentEmailHasher.generate(1)).should be_nil
    end
    it "should return nil if the hash is invalid" do
      SentEmailHasher.member_for("invalid_hash").should be_nil
    end
  end
end
